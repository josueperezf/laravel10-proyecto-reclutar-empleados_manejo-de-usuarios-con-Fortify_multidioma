{{--
    ...
    1. este formulario esta asociado directo a la clase EditarVacante,
    ...
    ademas de lo anterior, la clase 'EditarVacante', debe tener una variable o propiedad con el nombre del input al que va a controlar
    esto hace que cada vez que se coloque un caracter en la caja de texto, se hara una peticcion http,
    la misma llegara a la clase 'EditarVacante' donde se tendran las validaciones y demas
    ...
    para ello, en la etiqueta form debemos agregarle la propiedad "wire:submit.prevent='editarVacante'"
    'editarVacante' es el nombre del metodo de la clase 'EditarVacante' que procesara nuestro formulario al hacer submit
    ...
    4.  para poder enviar imagenes con livewire se debe colocar en la clase EditarVacante lo siguiente 'use WithFileUploads;'
...
<form class="md:w-1/2 space-y-5" wire:submit.prevent='editarVacante'>
    ...
    <!-- Imagen -->
    <div class="mt-2">
        <x-input-label for="imagen_nueva" :value="__('Image')" />
        <x-text-input id="imagen_nueva" class="block mt-1 w-full" type="file" wire:model="imagen_nueva" accept="image/*" />
        <x-input-error :messages="$errors->get('imagen_nueva')" class="mt-2" />
        <div class="my-5 w-80">
            {{-- recordemos que el __( es para decir que tradusca lo que esta dentro. ese texto lo busca en es.json --}}
            <x-input-label :value="__('Current image')" />
            {{--  la imagen la busca en storage/app/publica/vacantes --}}
            {{--  no se coloca $vacante->imagen si no $imagen, ya que lo que mostramos es el contenido de la clase EditarVacante.php y alli no existe el $vacante, existe el $imagen --}}
            {{--  si la imagen no se muestra y sabemos que esta, entonces debemos ejecutar en la terminal: sail artisan storage:link  para que donde de acceso al storage del proyecto desde el html--}}
            <img src="{{ asset('storage/vacantes/'.$imagen) }}"  />
        </div>

        <div class="my-5 w-80">
            @if ($imagen_nueva && !$errors->has('imagen_nueva'))
    ...
    {{--
        ...
        para solucionar en la clase EditarVacante se creó la propiedad 'cargando',
    ...
    <div class="mt-4">
        ...
        {{-- el siguiente bloque comentado hace que el boton submit se deshabilite solo cuando precionen en guardar, pero si un usuario esta adjunta una foto y de una vez da guardar no guarda la foto porque aun no la habia cargado, entonces mejor no ponerla, la deje comentado porque sirve de ejemplo como comunicarse con metodo backend desde el html  --}}
        {{-- <x-primary-button
            wire:click="cargador"
            wire:loading.attr="disabled"
